...
1. **Datos reales del directorio (DENUE)** _(en curso)_ — `pnpm importar-denue` (`packages/mcp-directorio/scripts/importar-denue.ts`) ya está verificado contra la API real: 4316 candidatos para Acámbaro (corrido por Armando desde su máquina, porque este entorno de trabajo bloquea el acceso saliente a `inegi.org.mx`). Falta completar a mano, por negocio, los campos que DENUE no trae (horario, WhatsApp, redes sociales, etiquetas) y decidir explícitamente, con consentimiento, cuáles pasan de `data/inegi/denue-candidatos.json` (staging, no versionado - PII real) a `src/data/negocios.json`. Detalle en `packages/mcp-directorio/data/inegi/README.md`.
